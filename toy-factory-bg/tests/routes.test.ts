...
vi.mock("@/lib/projects", () => ({ supabaseRest: vi.fn().mockResolvedValue([]), updateProject: vi.fn(), isModelKind: (v: string) => ["pop", "mini", "brick"].includes(v) }));
vi.mock("@/lib/jobs", () => ({ withProjectJob: vi.fn(), JobBusyError: class extends Error {} }));
...
it("does not restart production on duplicate paid events", async () => {
  vi.mocked(withProjectJob).mockImplementation(async (_id, work) => work(project({ paid_at: "2026-09-01", shopify_order_id: "gid://shopify/Order/1" })));
...
it("parks quantity mismatch instead of spending credits", async () => {
  vi.mocked(withProjectJob).mockImplementation(async (_id, work) => work(project()));
  await paid(order(2));
