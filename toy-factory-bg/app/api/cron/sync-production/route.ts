...
import { ensureShopifyWebhooks, shopifyAdminConfigured } from "@/lib/shopify-admin";
...
export async function GET(request: Request) {
  ...
  try {
    const shopifyWebhooks = shopifyAdminConfigured() ? await ensureShopifyWebhooks().catch((error) => {
      console.error(JSON.stringify({ event: "shopify_webhook_setup_failed", errorType: error instanceof Error ? error.name : "Error" }));
      return { error: error instanceof Error ? error.message : "Webhook setup failed" };
    ...
    return NextResponse.json({ ok: true, count: results.length, results, alerts, shopifyWebhooks });
  } catch (error) {
